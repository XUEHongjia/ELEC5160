...
# - link_library
################################################################################
set_app_var search_path ". /afs/ee.ust.hk/sun4x_56/usr/eelocal/tsmc_icdc/tsmc180/tsmc180_MS_RF_G/SC/tcb018g3d3/Rev280a/Front_End/timing_power_noise/CCS/tcb018g3d3_280a/ ../rtl $search_path"
...
elaborate design_top; # top module name

# store the unmapped results
write -hierarchy -format ddc -output results/design_top.unmapped.ddc
...
check_design > reports/design_top.check_design.rpt; # dump to the file
...
# Write out design
write -format verilog -hierarchy -output results/design_top.mapped.v
write -format ddc -hierarchy -output results/design_top.mapped.ddc
write_sdf results/design_top.mapped.sdf
write_sdc -nosplit results/design_top.mapped.sdc

# Generate reports
report_constraint > reports/design_top.mapped.constraint.rpt
report_qor > reports/design_top.mapped.qor.rpt
report_timing -transition_time -nets -attribute -nosplit -delay max \
  > reports/design_top.mapped.max_timing.rpt
report_area -nosplit > reports/design_top.mapped.area.rpt
report_power -nosplit > reports/design_top.mapped.power.rpt
